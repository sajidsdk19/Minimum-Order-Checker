...
class WC_Gateway_ThawaniGateway extends \WC_Payment_Gateway
{
    ...
    protected function prepare_products($order_id)
    {
        ...
        // Include order fees (e.g. Small Order Fee added by minimum-order-fee plugin)
        foreach ( $order_items->get_fees() as $fee_item ) {
            $fee_total = (float) $fee_item->get_total();
            if ( $fee_total > 0 ) {
                $products[] = [
                    'name'        => $fee_item->get_name(),
                    'unit_amount' => $this->format_price( $fee_total ),
                    'quantity'    => 1,
                ];
            }
        }
